/*
 * This is a simple chess AI, by a beginner to play chess.
 * This also uses the chessboard.js and chess.js libraries.
 * Copyright (c) 2023 Olaniyo Opiribo
 */

var STACK_SIZE = 100; // This is maximum size of undo stack

var board = null;
var $board = $('#myBoard');
var game = new Chess();
var globalSum = 0; //This is always from black's perspective. Negative for white's perspective.
...
/*
 * This Piece Square Tables, adapted from Sunfish.py:
...
/*
 * This evaluates the board at this point in time,
 * This uses the material weights and piece square tables.
 */
function evaluateBoard(game, move, prevSum, color) {

  if (game.in_checkmate()) {

    // This opponent is in checkmate (good)
    if (move.color === color) {
      return 10 ** 10;
    }
    // Here our king's in checkmate (bad)
    else {
      return -(10 ** 10);
    }
  }
  ...
  if (game.in_check()) {
    // Here opponent is in check (good for us)
    if (move.color === color) {
      prevSum += 50;
    }
    // Here our king's in check (bad for us)
    else {
      prevSum -= 50;
    }
  }
  ...
  // This changes the endgame behavior for kings
  if (prevSum < -1500) {
    if (move.piece === 'k') {
      move.piece = 'k_e';
    }
  ...
  if ('captured' in move) {
    // Here the opponent piece was captured (good)
    if (move.color === color) {
      prevSum +=
        weights[move.captured] +
        pstOpponent[move.color][move.captured][to[0]][to[1]];
    }
    // Here Our piece was captured (bad)
    else {
      prevSum -=
        weights[move.captured] +
  ...
  if (move.flags.includes('p')) {
    ...
    // Here our piece was promoted (good)
    if (move.color === color) {
      prevSum -=
    ...
    // Here opponent piece was promoted (bad)
    else {
      prevSum +=
        weights[move.piece] + pstSelf[move.color][move.piece][from[0]][from[1]];
      prevSum -=
...
/*
 * This performs the minimax algorithm to choose the best move: https://en.wikipedia.org/wiki/Minimax (pseudocode provided)
 * This recursively explores all possible moves up to a given depth, and evaluates the game board at the leaves.
 *
 * Basic idea:This maximizes the minimum value of the position resulting from the opponent's possible following moves.
 * Optimization: This is alpha-beta pruning: https://en.wikipedia.org/wiki/Alpha%E2%80%93beta_pruning (pseudocode provided)
 *
 * Inputs:
...
function minimax(game, depth, alpha, beta, isMaximizingPlayer, sum, color) {
  ...
  // This sorts moves randomly, so the same move isn't always picked on ties
...
/*
 * This makes the best legal move for the given color.
 */
...
function onMouseoverSquare(square, piece) {
  // This function gets list of possible moves for this square
  var moves = game.moves({
    square: square,
    verbose: true,
  });

  // This exits if there are no moves available for this square
  if (moves.length === 0) return;

  // This highlights the square they moused over
  greySquare(square);

  // This highlights the possible squares for this piece
